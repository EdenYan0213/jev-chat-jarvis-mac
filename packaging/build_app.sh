...
# A key that leaked into src/ would ship to whoever gets the bundle. Local builds carry
# no shared relay credentials, so every source file is checked without exceptions.
