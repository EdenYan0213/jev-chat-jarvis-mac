...
- 收发方向仍依赖文字位置，非对称布局可能误判；横跨左右区域或位于中间、无法确认方向的文本标为「方向未确认」，暂不作为回复目标，因此部分长消息可能被跳过。图片/表情包读不出内容；引用回复当普通文本；公众号文章卡片会被当消息解读
- 只有明确识别为「对方」的消息会触发判断与回复生成；只有自己消息或未确认文本时显示「等待可确认的对方消息…」，清空旧候选并丢弃过期异步结果。自己的消息仍可作为对话上下文。
- 发出消息与异步结果回归：`python -B -m unittest discover -s tests -v`（需已有项目运行依赖；使用合成 OCR 和模拟模型，不读取微信或调用 API）。
