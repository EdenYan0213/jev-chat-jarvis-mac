...
- **意图 + 风险 + 情绪**：覆盖 16 类职场与朋友聊天意图；会结合人物关系区分「派活 / 求助帮忙」「约会议 / 朋友邀约」，同时给出风险、主情绪、0–4 强度、变化趋势和行动建议
- **候选回复**：内置 13 种话术（含「暖心阳光男孩」「幽默男孩」「高情商幽默朋友」），每个已选话术生成 1 条，默认给出两个不同风格的候选；换话术会按当前消息重新生成
- **持续会话上下文**：双方可见消息按 Session 保存在本机；判断和生成使用同一份会话摘要 + 最近原文，长会话在后台自动压缩
- **本地模型调度**：判断与生成共用 Ollama 时，先等待消息稳定，再按顺序完成一次判断和候选生成，避免变化中的 OCR 文本和多个请求在同一模型前排队
- **YOLO 检测框**（可选，`JEV_BOXES=1` 启动即开、菜单栏可切）：OCR 命中的消息实时框在微信窗口上，对方/我分色 + 置信度

## 面板读法

面板使用 macOS 原生浅色磨砂材质：顶部是当前聊天、Session 下拉框、分析状态、正在处理的消息与上下文；中间依次显示意图、识别率、风险、情绪和行动建议；底部按话术分组展示候选回复。当前风险圆点会轻微呼吸提示。每条候选左侧是本地排序概率，右侧仍只有「复制」和「填入」；候选行会随完整文字自动增高，不截断内容，发送始终由用户在微信里手动完成。

「不用」的话术槽只保留一行下拉选择，不生成也不占候选行；开启或关闭话术只改变面板高度，不改变判断与轮询流程。黄色窗口按钮收起到聊天名与状态，红色窗口按钮退出。

## Session、上下文与本地记录

- 每个微信聊天可以有多个 Session。用面板顶部下拉框切换或选择「新建 Session…」；再次打开同一聊天时会恢复上次使用的 Session。切换会立即作废旧 Session 尚未返回的判断和候选，重新读取当前画面。
- 齿轮设置窗口顶部可切换「模型设置 / 会话管理」。会话页支持按会话名或聊天名搜索、重命名、移到回收站、恢复和永久删除；回收站保留 30 天。
- 应用只记录 OCR 当前实际看见的消息，不读取、注入或解密微信数据库，也不会为了补历史自动滚动聊天。当前画面里能看见的双方消息都会保存，群聊会同时保存可见的发送者名称。
- 数据库只在本机：`~/Library/Application Support/jev-jarvis/conversations.sqlite3`，文件权限为 `600`。删除应用不会自动删除这份记录。
- 判断和候选生成收到完全相同的 Session 上下文：滚动摘要加最近 20 条原文。上下文超过约 8,000 字符时，配置可用的生成模型会在后台更新摘要；原始消息不会因摘要而删除。单次模型输入仍有 12,000 字符硬上限。
- 如果 Jev 或候选生成连接的是外部服务，当前消息及上述受限上下文会发送给对应服务；配置本机 Laya/Ollama 端点时则保留在本机。
- 所有操作仍保持只读安全边界：应用不会自动发送消息，也不会自动滚动微信。「填入」只在用户明确点击后写入草稿，最终发送仍由用户完成。
...
点击悬浮窗右上角 **齿轮图标（模型设置）**，或菜单栏 **J → 模型设置…**，可编辑 Jev、OpenAI 兼容、Anthropic 兼容三组密钥、服务地址与模型；窗口顶部的「会话管理」可管理本地 Session。
...
- 密钥掩码显示；窗口仅读取所编辑文件中的值，不把环境变量或项目 `.env` 的密钥复制进用户文件。各配置页顶部显示本次启动正在使用自己的密钥、本地判断或尚未配置，以及实际来源；生成页同时标明当前启用的服务。
- 环境变量优先于用户 env，用户 env 优先于项目 `.env`；生成层 OpenAI 组优先于 Anthropic 组。清空当前文件的密钥不会禁用其他来源中的密钥。由终端或启动器导出的值也显示为「环境变量」。
...
export OPENAI_EXTRA_BODY='{"reasoning_effort":"none"}'
...
- Session 只能积累应用实际看见过的内容；没有滚动到屏幕里的旧消息不会自动补入。OCR 无法和历史重叠时会保留一个记录缺口标记，并从当前画面继续。
